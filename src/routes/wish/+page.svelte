<script lang="ts">
	...
	function mapError(reason: 'no_banner' | 'insufficient_primo' | 'novice_maxed'): string {
		if (reason === 'no_banner') return 'Pilih banner dulu.';
		if (reason === 'novice_maxed') return 'Novice Wish sudah mencapai limit 20 pull.';
		const tenCost = game.wishMode === 'novice' ? game.NOVICE_COST_TEN : game.COST_TEN;
		return `Primogem tidak cukup untuk 10-pull (butuh ${tenCost.toLocaleString('en-US')}).`;
	}
			...
			<div class="relative h-64 md:h-80 lg:h-96 flex items-end justify-center">
				{#if game.wishMode === 'character'}
					...
					{:else if banners.isLoading}
						<div class="absolute inset-0 flex items-center justify-center text-[#8E97AA] text-sm animate-pulse">Memuat banner…</div>
					{:else}
						<div class="absolute inset-0 flex items-center justify-center text-[#8E97AA] text-sm">Tidak ada banner tersedia.</div>
					{/if}
				{:else if game.wishMode === 'standard'}
					...
					<div class="absolute inset-0 flex flex-col items-center justify-center" in:fly={{ y: 20, duration: 500, easing: cubicOut }}>
						...
						<div class="relative text-center space-y-3">
							...
							<div class="font-heading text-2xl font-bold text-[#F2E6D0]">Wanderlust Invocation</div>
							<div class="text-xs text-[#8E97AA] uppercase tracking-wider">Permanent Banner</div>
							<div class="text-[11px] text-[#B8C1D3] max-w-xs mx-auto leading-relaxed pt-2">
								No rate-up, no 50/50. Pool 5★: {standardCharCount} characters + {standardWeaponCount} weapons (50/50 split).
							</div>
						</div>
					</div>
