...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
	...
	<section class="bg-[#1A2337]/80 backdrop-blur-sm rounded-xl border border-[#C9A45A]/25 shadow-xl p-5 space-y-4">
		...
		<!-- Recipe -->
		<div class="space-y-2">
			<label for="recipe" class="text-xs font-bold text-[#E6C77A] uppercase tracking-wider">Recipe</label>
			<select
				id="recipe"
			...
			>
				{#each RECIPES as r}
					<option value={r.id}>{r.name} ({r.stars}★)</option>
				{/each}
			</select>
		...
		<!-- Character -->
		<div class="space-y-2">
			<label for="char" class="text-xs font-bold text-[#E6C77A] uppercase tracking-wider">Character</label>
			<select
				id="char"
				bind:value={selectedCharId}
				class="w-full px-3 py-2 rounded-md bg-[#0B1020] border border-[#24314A] text-[#F2E6D0] text-sm focus:outline-none focus:border-[#C9A45A]/60"
			>
				{#each CHAR_OPTIONS as c}
