	...
	<section class="bg-[#1A2337]/80 backdrop-blur-sm rounded-xl border border-[#C9A45A]/25 shadow-xl p-5 space-y-3">
		...
		<div class="flex gap-2">
			<input
				...
				class="flex-1 px-3 py-2 rounded-md bg-[#0B1020] border border-[#24314A] text-[#F2E6D0] font-mono text-sm placeholder-[#5E6478] focus:outline-none focus:border-[#C9A45A]/60 focus:ring-1 focus:ring-[#C9A45A]/40"
...
{#if showManualModal}
	...
	<div
		...
		onkeydown={(e) => { if (e.key === 'Escape') showManualModal = false; }}
		transition:fade
	>
		<div class="max-w-sm w-full rounded-xl border border-[#C9A45A]/40 bg-gradient-to-br from-[#1A2337] to-[#0B1020] p-6 shadow-2xl" in:fly={{ y: 20, duration: 200, easing: cubicOut }}>
			<div class="text-center mb-4">
				...
				<h3 class="font-heading text-lg font-semibold text-[#F2E6D0]">Konfirmasi Primo</h3>
			...
			<input
				...
				step="160"
				value={manualPrimo ?? 0}
				oninput={(e) => manualPrimo = Math.max(0, parseInt(e.currentTarget.value, 10) || 0)}
				class="w-full px-3 py-2 rounded-md bg-[#0B1020] border border-[#24314A] text-[#F2E6D0] font-mono text-sm text-center focus:outline-none focus:border-[#C9A45A]/60 focus:ring-1 focus:ring-[#C9A45A]/40 mb-4"
				placeholder="0"
			/>
			<div class="flex gap-2">
				<button
