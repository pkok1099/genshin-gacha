	...
	<section class="flex flex-col md:flex-row md:items-end justify-between gap-4">
		...
		<div class="flex gap-2">
			...
			<button
				...
				class="px-4 py-2 rounded-md border border-[#8B3A3A]/40 bg-[#8B3A3A]/15 text-[#E8745A] text-xs font-semibold uppercase tracking-wider hover:bg-[#8B3A3A]/25 transition-all"
			>
				Reset History
			</button>
		</div>
...
{#if confirmReset}
	...
	<div
		...
		onclick={(e) => { if (e.target === e.currentTarget) confirmReset = false; }}
		onkeydown={(e) => { if (e.key === 'Escape') confirmReset = false; }}
		transition:fade
	>
		<div class="max-w-sm w-full rounded-xl border border-[#8B3A3A]/40 bg-gradient-to-br from-[#1A2337] to-[#0B1020] p-6 shadow-2xl">
			<h3 class="font-heading text-lg font-semibold text-[#F2E6D0] mb-2">Reset History?</h3>
