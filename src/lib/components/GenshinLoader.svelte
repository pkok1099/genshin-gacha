<script lang="ts">
	...
	// Shows gold wish swirl + sparkle particles + "Loading..." text
...
<div class="genshin-loader fixed inset-0 z-[60] flex items-center justify-center bg-[#0B1020] pointer-events-auto">
	...
	<div class="relative flex flex-col items-center justify-center gap-6">
		...
		<div class="w-48 h-0.5 bg-[#1A2337] rounded-full overflow-hidden">
			<div class="h-full bg-gradient-to-r from-[#C9A45A] via-[#E6C77A] to-[#C9A45A] animate-progress"></div>
		</div>
	</div>
	...
	<div class="absolute inset-0 pointer-events-none overflow-hidden">
		{#each Array(12) as _, i}
			<div
				class="absolute w-1 h-1 rounded-full bg-[#E6C77A] opacity-0 animate-sparkle"
				style="
					left: {15 + (i * 7) % 70}%;
					top: {20 + (i * 11) % 60}%;
